...
CONFIG = {
    "data_path": "nursery.csv",        # [cite: 1] referenced via path
    "target_col": "Target",         # [cite: 1]
    "seed": 42,
    "n_teachers": 4,                # Increased back to 5 for more robust aggregation (adjust if needed)
    "teacher_lr": 1e-5,             # Standard teacher LR
    "teacher_epochs": 500,           # Increased teacher epochs for better learning
    "t_hidden_dim": 2000,            # Slightly increased teacher capacity
    "lap_scale": 0.01,             # *** VERY IMPORTANT: Start very low. STRONGLY recommend testing lap_scale = 0.0 first! ***
    "noise_dim": 100,               # Reduced noise dimension slightly
    "g_hidden_dim":2048,            # Kept G/S capacity moderate
    "s_hidden_dim":256,            # Kept G/S capacity moderate
    "pate_gan_epochs": 100,         # Increased PATE-GAN training epochs
    "batch_size": 128,
    "g_lr": 3e-5,                   # Adjusted G LR (Try 1e-4, 5e-5, 1e-5 as alternatives)
    "s_lr": 1e-6,                   # Adjusted S LR (Try making it equal to g_lr, or 1/10th of g_lr)
    "log_every": 5,                # Log every 20 epochs
...
class ConditionalGenerator(nn.Module):
    ...
    def __init__(self, noise_dim: int, data_dim: int, num_classes: int, hidden_dim: int = 256):
        ...
        self.net = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.2),  # Light dropout (optional)

            nn.Linear(hidden_dim * 2, hidden_dim),   # Contract back
            nn.BatchNorm1d(hidden_dim),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Linear(hidden_dim, hidden_dim // 2),  # Contract further
            nn.BatchNorm1d(hidden_dim // 2),
            nn.LeakyReLU(0.2, inplace=True),

            nn.Linear(hidden_dim // 2, data_dim),
            nn.Tanh()  # Output layer matches data scaling
        )
...
def evaluate_tstr(X_syn: np.ndarray, y_syn: np.ndarray,
    ...
    models = {
        ...
        "MLP": MLPClassifier(hidden_layer_sizes=(256, 128,32), max_iter=500, random_state=CONFIG['seed'], early_stopping=True),
        "XGBoost": xgb.XGBClassifier(eval_metric='logloss', random_state=CONFIG['seed']),
    }
